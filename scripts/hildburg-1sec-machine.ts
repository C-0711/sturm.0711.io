...
const META_DIR  = process.env.META_DIR ?? "/home/christoph.bertsch/0711/0711-STURM/workspaces/haubrich-koch-hildburg-2024/meta";
...
const OUTPUT    = process.env.OUTPUT ?? "/tmp/hildburg-1sec.json";
...
async function embedAll(kpis: KPI[]) {
  ...
  // Parallel batching: 4 chunks á ~150 queries each
  const EMBED_CHUNKS = 8;
  const chunkSize = Math.ceil(queries.length / EMBED_CHUNKS);
  const chunks: string[][] = [];
  for (let i = 0; i < queries.length; i += chunkSize) chunks.push(queries.slice(i, i + chunkSize));
  const results = await Promise.all(chunks.map(c => embedBatch(c)));
...
async function tier2Decide(kpi: KPI, candidates: any[]): Promise<{ pick: string | null; reason: string }> {
  // Micro-prompt: A/B/C/N only. guided_choice constrains decode to single token.
  const candStr = candidates.map((c: any, i: number) => `${String.fromCharCode(65+i)}=${c.drucktext}`).join(" | ");
  const userPrompt = `${kpi.belegtyp} "${kpi.key}"=${typeof kpi.value === "string" ? kpi.value.slice(0,40) : kpi.value} | ${candStr} | N=keine. Antwort: nur Buchstabe.`;
  try {
    const resp = await fetch(VLLM_URL, {
      ...
      body: JSON.stringify({
        ...
        max_tokens: 2,
        temperature: 0,
        guided_choice: ["A","B","C","N"],
      }),
    });
    if (!resp.ok) return { pick: null, reason: `http ${resp.status}` };
    const data = await resp.json();
    const out = (data.choices?.[0]?.message?.content ?? "").trim().toUpperCase();
    const letter = out.match(/[ABCN]/)?.[0];
    if (!letter || letter === "N") return { pick: null, reason: letter === "N" ? "none" : `unparseable:${out.slice(0,20)}` };
    const idx = letter.charCodeAt(0) - 65;
    return { pick: candidates[idx]?.ecode ?? null, reason: letter };
  } catch (e: any) {
...
async function main() {
  ...
  // Stage 2.5: Cross-Doc-Verification ─ eCodes die in ≥2 Belegen mit score ≥0.50 als TOP auftauchen, auto-accept
  const tCD0 = performance.now();
  const CROSSDOC_MIN_SCORE = 0.50;
  const CROSSDOC_MIN_DOCS = 2;
  const ecodeDocs: Record<string, Set<string>> = {};
  for (const m of med) {
    const top = m.matches[0];
    if (!top || top.score < CROSSDOC_MIN_SCORE) continue;
    if (!ecodeDocs[top.ecode]) ecodeDocs[top.ecode] = new Set();
    ecodeDocs[top.ecode].add(m.kpi.doc);
  }
  const autoAcceptEcodes = new Set(Object.keys(ecodeDocs).filter(ec => ecodeDocs[ec].size >= CROSSDOC_MIN_DOCS));
  const autoAccepted = med.filter(m => autoAcceptEcodes.has(m.matches[0]?.ecode));
  const remaining = med.filter(m => !autoAcceptEcodes.has(m.matches[0]?.ecode));
  console.log(`[CrossDoc]  ${(performance.now()-tCD0).toFixed(0)} ms · ${autoAcceptEcodes.size} eCodes auto-accepted (${autoAccepted.length} matches), ${remaining.length} for Tier-2`);

  // Stage 3: Tier-2 disambig (cap on top-N to stay in 1s budget)
  const TIER2_CAP = 60;
  const medSortedTop = [...remaining].sort((a, b) => (b.matches[0]?.score ?? 0) - (a.matches[0]?.score ?? 0)).slice(0, TIER2_CAP);
  ...
  // Add cross-doc auto-accepted (skipped Tier-2)
  for (const m of autoAccepted) {
    const ec = m.matches[0].ecode;
    if (!canonical[ec]) canonical[ec] = { ecode: ec, anlage: m.matches[0].anlage, drucktext: m.matches[0].drucktext, vordruckzeile: m.matches[0].vordruckzeile, values: [] };
    canonical[ec].values.push({ value: m.kpi.value, source_doc: m.kpi.doc, source_kpi_key: m.kpi.key, score: m.matches[0].score, tier: "cross-doc" });
  }
  // Add tier2 accepted
