...
export const PROJECTION_RULES: ProjectionRule[] = [
  ...
  // ───────────────────────────────────────────────────────────────────────────
  // Phase F · Kapitalertrag-Cluster (Anlage KAP)
  // ───────────────────────────────────────────────────────────────────────────
  // mitteilung_kapitalertraege = VaSt-Auszug (Sparkasse, Volksbank, …)
  //   → Nur freigestellter Betrag via Freistellungsauftrag.
  //   → KEINE einbehaltene KapESt.
  //   → Aggregat geht in KAP E1901401 (in Anspruch genommener Sparer-Pauschbetrag)
  //     bzw. E1901402 für Anlage-KAP-fremde Kapitalerträge.
  //
  // steuerbescheinigung_kapitalertraege = Volle Bankbescheinigung nach § 45a EStG
  //   → Höhe der KapErt + abgezogene Steuern (KapESt, SolZ, KiSt).
  //   → Direkte 1:1 Mappings auf Anlage-KAP-Felder.

  // ── mitteilung_kapitalertraege → Sparer-Pauschbetrag (E1901401) ──
  {
    targetCode: 'E1901401',
    applicableDocClasses: ['mitteilung_kapitalertraege'],
    sourcePath: 'summen.summe_freigestellt_eur',
    aggregator: 'first',
    description: 'In Anspruch genommener Sparer-Pauschbetrag (Anlage KAP Zeile 16) — aus VaSt-Mitteilung freigestellte Erträge',
    rechtsgrundlage: '§ 20 Abs. 9 EStG',
  },
  // Fallback: wenn summen-Aggregat fehlt, summiere freigestellte_kapitalertraege[].betrag_eur
  {
    targetCode: 'E1901401',
    applicableDocClasses: ['mitteilung_kapitalertraege'],
    sourcePath: 'freigestellte_kapitalertraege',
    filter: (e: any) => typeof e?.betrag_eur === 'number' && e.betrag_eur > 0,
    aggregator: 'sum',
    description: 'Sparer-Pauschbetrag (Fallback aus freigestellte_kapitalertraege[])',
    rechtsgrundlage: '§ 20 Abs. 9 EStG',
  },

  // ── steuerbescheinigung_kapitalertraege → Anlage-KAP-Felder ──
  {
    targetCode: 'E1900701',
    applicableDocClasses: ['steuerbescheinigung_kapitalertraege'],
    sourcePath: 'kapitalertraege.hoehe_kapitalertraege_inland_eur',
    aggregator: 'first',
    description: 'Kapitalerträge (KAP Zeile 7) — inländische Kapitalerträge der KapESt unterlegen',
    rechtsgrundlage: '§ 32d Abs. 1 EStG',
  },
  {
    targetCode: 'E1900702',
    applicableDocClasses: ['steuerbescheinigung_kapitalertraege'],
    sourcePath: 'kapitalertraege.hoehe_kapitalertraege_ausland_eur',
    aggregator: 'first',
    description: 'Kapitalerträge (KAP Zeile 7 Ausland) — ausländische Kapitalerträge ohne KapESt',
    rechtsgrundlage: '§ 32d Abs. 1 EStG',
  },
  {
    targetCode: 'E1901501',
    ...
    sourcePath: 'kapitalertraege.hoehe_kapitalertraege_inland_eur',
    aggregator: 'first',
    description: 'Inländische Kapitalerträge (KAP Zeile 18, ohne Beträge laut Zeilen 24-26a)',
    rechtsgrundlage: '§ 32d Abs. 1 EStG',
  },
  {
    targetCode: 'E1901702',
    ...
    sourcePath: 'kapitalertraege.hoehe_kapitalertraege_ausland_eur',
    aggregator: 'first',
    description: 'Ausländische Kapitalerträge (KAP Zeile 19)',
    rechtsgrundlage: '§ 32d Abs. 1 EStG',
  },
  {
    targetCode: 'E1901401',
    applicableDocClasses: ['steuerbescheinigung_kapitalertraege'],
    sourcePath: 'kapitalertraege.in_anspruch_genommener_sparer_pauschbetrag_eur',
    aggregator: 'first',
    description: 'In Anspruch genommener Sparer-Pauschbetrag (KAP Zeile 16)',
    rechtsgrundlage: '§ 20 Abs. 9 EStG',
  },
  {
    targetCode: 'E1904701',
    applicableDocClasses: ['steuerbescheinigung_kapitalertraege'],
    sourcePath: 'abgezogene_steuern.kapitalertragsteuer_eur',
    aggregator: 'first',
    description: 'Kapitalertragsteuer (KAP Zeile 37) — von der Bank einbehaltene KapESt',
    rechtsgrundlage: '§ 43 Abs. 5 EStG',
  },
  {
    targetCode: 'E1904801',
    applicableDocClasses: ['steuerbescheinigung_kapitalertraege'],
    sourcePath: 'abgezogene_steuern.kirchensteuer_eur',
    aggregator: 'first',
    description: 'Kirchensteuer zur Kapitalertragsteuer (KAP) — nur wenn kirchensteuerpflichtig',
    rechtsgrundlage: '§ 51a Abs. 2c EStG',
  },

  // ── religionszugehoerigkeit → Kirchensteuer-Flag (E1900601) ──
  {
    targetCode: 'E1900601',
    applicableDocClasses: ['religionszugehoerigkeit'],
    sourcePath: 'personen',
    filter: (p: any) => {
      const k = String(p?.konfession ?? '').toLowerCase();
      return ['evangelisch', 'katholisch', 'rk', 'ev', 'altkatholisch', 'jüdisch', 'juedisch'].some((x) => k.includes(x));
    },
    aggregator: 'count',
    description: 'Kirchensteuerpflicht-Flag (KAP Zeile 6) — count > 0 ⇒ kirchensteuerpflichtig, einbehaltene KiSt zur KapESt überprüfen',
    rechtsgrundlage: '§ 51a Abs. 2c EStG',
  },
];
