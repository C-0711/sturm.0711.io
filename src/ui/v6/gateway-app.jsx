...
/* ========== K4: useKlavisHealth — live probe to /api/klavis/health ========== */
function useKlavisHealth(intervalMs = 60000) {
  const [state, setState] = useState({ truth: 'inferred', model: null, ready: null, ageMs: null, error: null });
  useEffect(() => {
    let cancelled = false;
    const probe = async () => {
      try {
        const r = await fetch('https://gateway.0711.io/api/klavis/health');
        const j = await r.json();
        if (!cancelled) setState({
          truth: j.truth || 'unknown',
          model: j.data && j.data.model ? j.data.model.replace('anthropic/', '').replace('openai/', '') : null,
          ready: j.data ? j.data.ready : null,
          ageMs: j.ageMs != null ? j.ageMs : null,
          error: (j.data && j.data.error) ? j.data.error : null,
        });
      } catch (e) {
        if (!cancelled) setState(s => ({ ...s, truth: 'unavailable', error: e.message }));
      }
    };
    probe();
...
function Overview({ onNav }) {
  const klavisHealth = useKlavisHealth();
      ...
      <div className="klavis-card">
        ...
        <div className="klavis-card-grid">
          <div className="klavis-card-cell">
            ...
            <span className="val">{klavisHealth.ready === false ? <span className="s-dot err" /> : <span className="s-dot ok" />} {klavisHealth.model || 'Opus 4.7'}</span>
          </div>
          <div className="klavis-card-cell">
